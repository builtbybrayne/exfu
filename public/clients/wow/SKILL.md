...
- `context/me/tools.md` — *(if Phase 3 has run)* the user's tool inventory, what's connected via MCP, what isn't.
- `context/me/writing-style.md` — *(if Phase 6 has run)* the user's writing voice profile.
...
- *(List scope skills + folder paths as they're created, e.g. "`scope-acme-deal` → `scopes/acme-deal/`". This grows during Phase 5 and beyond.)*
