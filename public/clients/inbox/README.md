...
Manages a lightweight quick-capture file in the user's substrate at `databases/inbox/inbox.md`. Appends, reviews, sorts, and processes items. Called by the `substrate` skill on session load to flag that there are pending items.
...
- `substrate` skill installed (substrate delegates inbox checks to this skill on load)
