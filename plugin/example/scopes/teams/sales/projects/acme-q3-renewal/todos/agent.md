...
This file was copied from `exfu/ontologies/todos/agent.md` on 2026-04-15 when this scope was created. The folder follows this snapshot, not whatever the upstream template says today.

Note that `user/todos/agent.md` was copied later (2026-05-21). The two snapshots happen to be identical in content because the upstream template did not change between those dates, but they are independent copies and can diverge if either is refreshed or hand-edited.

## Scope-specific note

Todos here belong to the Acme Q3 renewal only. Cross-scope personal todos live in `user/todos/`. The agent surfaces them separately when Alastair asks "what's on my plate?".

## Dependencies

- `exfu/ontologies/todos/schema.yaml` — the schema this convention enforces.
- The user's `al-todos` skill (in `~/.claude/skills/`) — handles operations against this file.
