...
def render_librarian_dashboard(registry_data, log_data):
    ...
    if registry_data is None:
        return (
            '<div class="empty-state">'
            "<h3>No librarian registry found</h3>"
            "<p>The librarian framework has not been set up yet. "
            "Once librarians are registered, their health will appear here.</p>"
            "</div>"
        )

    librarians = registry_data.get("librarians", [])
    cadences = registry_data.get("cadences", {})

    if not librarians:
        return (
            '<div class="empty-state">'
            "<h3>No librarians registered</h3>"
            "<p>The registry exists but has no librarians. "
            "Register a librarian to see it here.</p>"
            "</div>"
        )
...
<body>
  <div class="container">
    ...
    <div class="tabs">
      ...
      <button class="tab" data-tab="tab-librarians">Librarians</button>
